...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const t = await getTranslations({ locale, namespace: 'pages.creation_site' });
...
export default async function CreationSiteFrancePage({ params }: Props) {
  const { locale } = await params;
  const tMeta = await getTranslations({ locale, namespace: 'pages.creation_site' });
  const t = await getTranslations({ locale, namespace: 'creation_site_content' });

  const forItems      = t.raw('section_for_items')      as string[];
      ...
      <div className="py-20 px-4 sm:px-6 lg:px-8 bg-slate-950 min-h-screen">
        <div className="max-w-3xl mx-auto">
          ...
          <section className="mb-12 card p-6 rtl:text-right">
            <h2 className="text-xl font-bold text-white mb-3">{t('section_timeline_title')}</h2>
            <p className="text-slate-400 leading-relaxed">{t('section_timeline_text')}</p>
          </section>
          ...
          <div className="card p-8 text-center rtl:text-right">
            <p className="font-bold text-white text-xl mb-2">Prêt à lancer votre site ?</p>
            <p className="text-slate-400 mb-6">Devis détaillé sous 48h, sans engagement.</p>
