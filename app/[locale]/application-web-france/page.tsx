...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const t = await getTranslations({ locale, namespace: 'pages.app_web' });
...
export default async function ApplicationWebFrancePage({ params }: Props) {
  const { locale } = await params;
  const tMeta = await getTranslations({ locale, namespace: 'pages.app_web' });
  const t = await getTranslations({ locale, namespace: 'app_web_content' });

  const typesItems   = t.raw('section_types_items')   as string[];
      ...
      <div className="py-20 px-4 sm:px-6 lg:px-8 bg-slate-950 min-h-screen">
        <div className="max-w-3xl mx-auto">
          ...
          <div className="card p-8 text-center rtl:text-right">
            ...
            <p className="text-slate-400 mb-6">Parlons-en — réponse sous 24h, sans engagement.</p>
